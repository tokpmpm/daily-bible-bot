...
def get_daily_verse(now=None):
    ...
    votd_url = f"{API_BASE}/verse_of_the_days/{day}"
    votd = _request_json(votd_url, headers)
    passage_id = votd.get("passage_id", "").strip()
    if not passage_id:
        raise ValueError(f"YouVersion VOTD response has no passage_id: {votd}")

    passage_url = f"{API_BASE}/bibles/{CUNP_VERSION_ID}/passages/{passage_id}"
    passage = _request_json(passage_url, headers)
